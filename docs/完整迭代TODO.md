...
- [x] **F-04 评论工作区**：分页、排序、加载更多、回复和真实总数已接入；错误重试仍待补。
...
- [x] **R-01 前端 CI**：本地 lint、Vitest（6 个文件/8 个测试）和生产 build 全部通过；CI 接入仍待补。
- [ ] **R-02 后端 CI**：gofmt、vet、test、race、build、MySQL/Redis 集成、迁移矩阵。
- [x] **R-03 契约 CI**：分页、公开 DTO、举报枚举和交互类型已统一并有后端/前端回归测试；CI 工作流接入仍待补。
