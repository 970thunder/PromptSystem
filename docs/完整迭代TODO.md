...
- Compose 项目：`promptsystem`；发布目录：`/srv/releases/promptsystem/20260829-a9ba2cf`
...
- [ ] **P0-01 正式生产环境**：生产设置 `APP_ENV=production`；开发、测试、生产判断集中到配置包。验收：生产不会返回 `devCode`，生产校验测试通过。
- [ ] **P0-02 禁止生产内存降级**：MySQL、迁移或生产初始化失败时启动失败，不接受会在重启后消失的写入。验收：生产依赖失败测试覆盖；ready 非 200。
...
- [ ] **P0-05 公开用户隐私拆分**：公开用户 DTO 不含邮箱、OAuth 绑定和账户状态；本人 DTO 单独返回私有字段。验收：Prompt/评论/关注公开响应无邮箱。
- [ ] **P0-06 验证码生产安全**：接入邮件发送；验证码不在生产响应/日志出现，Redis 只存哈希并原子消费。验收：发送、过期、重放、并发、限流测试通过。
- [ ] **P0-07 独立数据库账号**：应用不再以 MySQL root 运行，迁移与运行权限分离。验收：最小权限账号可正常运行，越权 DDL/跨库访问失败。
- [ ] **P0-08 评论分页契约统一**：前端使用 `{list,total,page,pageSize}`。验收：线上空评论、非空评论、加载更多和排序均正确。
- [ ] **P0-09 浏览历史分页契约统一**：工作台适配分页响应。验收：历史列表不再被错误清空，分页/软删除过滤正确。
- [ ] **P0-10 举报枚举修复**：前端只发送 `spam/abuse/nsfw/other`。验收：Prompt/评论举报、重复举报、非法枚举测试通过。
- [ ] **P0-11 互动闭环**：接入 interaction、unlike、unfavorite，显示真实选中态。验收：刷新后状态正确且计数幂等。
...
- [ ] **A-08 数据库连接池**：配置最大/空闲连接与生命周期，并采集连接池指标。
...
- [ ] **D-01 上传所有者约束**：`uploads.owner_id` 建立用户引用或等价强校验。
...
- [ ] **S-03 GitHub OAuth 开关**：显式 enable 配置；未配置时前端不显示可点击入口，启用时校验 redirect/state/code。
...
- [ ] **F-01 匿名浏览计数**：未登录阅读详情也调用匿名友好的 view 接口。
- [ ] **F-02 公开作者页**：`/profile/:userId` 可匿名访问，只有 `/profile` 本人工作台需要登录。
- [ ] **F-03 真正退出登录**：UI 调用 `/user/logout` 后清理本地状态并返回安全页面。
- [ ] **F-04 评论工作区**：分页、排序、加载更多、回复、错误重试和真实总数。
- [ ] **F-05 相关推荐**：通过稳定后端查询获取，不依赖当前 Pinia 页缓存。
...
- [ ] **O-02 无 Swap 条件下的发布预算**：镜像 load、备份、数据库迁移串行，不并行运行高内存任务。
- [ ] **O-03 Docker 日志轮转**：单容器限制大小与份数，避免日志无上限增长。
...
- [ ] **O-08 端口边界**：公网仅 80/443；3092/5092/MySQL/Redis/RustFS 内部端口持续检查。
- [ ] **O-09 多项目隔离**：发布脚本不得停止、重建或删除其他 Compose 项目和卷。
- [ ] **O-10 故障演练**：backend/MySQL/Redis/RustFS/nginx/证书/磁盘逐项演练并记录恢复。

## R 测试、CI 与发布

- [ ] **R-01 前端 CI**：lint、Vitest、生产 build。
- [ ] **R-02 后端 CI**：gofmt、vet、test、race、build、MySQL/Redis 集成、迁移矩阵。
- [ ] **R-03 契约 CI**：分页、错误码、公开 DTO 和前端类型一致性。
