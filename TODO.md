...
- [x] 设计暗黑主题
- [x] 设计响应式规则
- [x] 整理 Tailwind theme tokens

### 2.2 基础组件

> 基础组件按 Naive UI + Tailwind theme token 复用闭环，不额外新建平行组件库。
...
- [x] 搜索建议
- [x] 搜索历史
- [!] ElasticSearch 接入（项目规则约定为 Phase 2+，当前 Phase 1 不实现）
...
- [x] Prompt 收藏
- [x] 关注创作者
- [!] 消息通知（需确认通知类型、接收人、已读状态、投递方式与保留周期）
- [!] 系统通知（需确认系统通知来源、权限边界、投递方式与保留周期）
...
- [!] 数据统计（需确认创作者中心指标口径、时间窗口与权限边界）
- [!] Prompt 管理（需确认创作者后台范围是否复用个人主页或新增后台入口）
- [!] Skill 管理占位（Skill 管理属于 Phase 2+，当前只做首页分类入口占位）
- [!] 粉丝管理（需确认创作者后台入口、列表权限与分页规则）
- [!] 收益统计占位（收益属于商业化能力，需确认 Phase 与数据口径）
- [!] 发布统计（需确认统计周期、数据来源与创作者权限）

### 10.2 数据分析

- [!] 浏览量（需确认创作者维度统计周期与聚合方式）
- [!] 点赞量（需确认创作者维度统计周期与聚合方式）
- [!] 收藏量（需确认创作者维度统计周期与聚合方式）
- [!] 复制次数（需先定义复制埋点与去重规则）
- [!] 转化率（需先定义转化目标与归因规则）
- [!] 用户增长（需先定义增长口径与权限边界）
...
- [x] XSS 防护
- [!] CSRF 防护（当前 JWT Bearer 方案下需确认 Cookie 使用场景与跨站提交边界）
- [!] 接口限流（需确认限流维度、阈值、Redis 依赖与异常处理策略）
